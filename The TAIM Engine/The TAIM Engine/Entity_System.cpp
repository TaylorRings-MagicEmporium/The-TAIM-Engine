...
Entity* Entity_System::CreateEntity(glm::vec3 Position, glm::quat Rotation, glm::vec3 Scale, std::string tag = "undefined", std::string FileTag = "undefined",  std::string name = "undefined") {
	Entity* object = new Entity(Position, Rotation, Scale, tag, FileTag, name);
...
Entity* Entity_System::GetEntityWithName(std::string name)
{
	for (int i = 0; i < ALL_ENTITIES.size(); i++) {
		if (ALL_ENTITIES[i]->GetName() == name) {
